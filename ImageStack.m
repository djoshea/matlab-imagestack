...
%   s.everyNthFrame(50).normalize.montage
    ...
    methods % Transformations
        function s = alignToReferenceTranslation(s, reference)
            [optimizer, metric] = imregconfig('monomodal');
            prog = ProgressBar(s.nFrames, 'Aligning images...');
            for i = 1:s.nFrames
                prog.update(i);
                s.data(:, :, :, i) = imregister(s.data(:, :, :, i), reference, 'translation', optimizer, metric);
            end
            prog.finish();
        end
    ...
    methods % Visualization methods
        function setColormap(s)
            colormap(ImageStack.getDefaultColormap());
        end
        
        function imcat(s)
           m = s.getFrames(1);
           C = ImageStack.getDefaultColormap;
    ...
    methods(Static)
        function s = fromAllInDirectory(imgDir, varargin)
            ...
            img = ds.readimage(1);
            [useRegion, cropX, cropY, szImg] = ImageStack.determinePixelRegion(p.Results.cropX, p.Results.cropY, size(img));
            ...
            s = ImageStack(stack, 'name', name, p.Unmatched);
        end
